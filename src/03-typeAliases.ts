...
function getBuddyListFriends(list: BuddyList): Friend[] {
  const friends = list.members.reduce((friendsInBuddyList: Friend[], buddy: Buddy) => {
    if ('phone' in buddy) { // Check if 'phone' exists in the buddy since only friends have phone property
      friendsInBuddyList.push({ name: buddy.name, phone: buddy.phone, age: buddy.age});
    }
    return friendsInBuddyList;
  }, []);
  return friends;
}
